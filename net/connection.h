...
public:
  ...
  **/
  ...
  /** 
		Handler class used to inform the client for events.
			...
			- on_message_sent should be invoked when a message was sent successfully using
				async_send.
			TODO(kari): What happens if there is an error; how error pass the message id
			- on_connected should be invoked when the connection with the remote peer was 
				established.
			- on_disconnected should be invoked when connection was closed/ destroyed by 
				the local or the remote peer
			- on_error should be invoked when an error happens while listening or accepting.
	**/
	class connection_handler {
	public:
		virtual void on_message_received(connection* c, const std::string& message) = 0;
  ...
  **/
  connection(connection_handler* _handler);
	...
	**/
  virtual void async_send(const std::string& message, int id) = 0;// return int == sequence_id
  ...
  /**
		...
		factory_function specifies the function tht will be used to create an object.
		See "typedef connection* (*factory_function)..." above to check what arguments 
		should this function accept. If such type name exists in the registry, the 
		factory_function pointer will be overriden.
	**/
  static void register_connection_type(const std::string& type, factory_function func);
	...
	/**
		...
		messages that should be resent. It increments with one every time is invoked.
